...
// (price - unitCost × exchangeRate × markup) / ( unitCost × exchangeRate × markup) × 100
function calculateMargin(item: TargetPriceItem) {
  // const { unit_price_no_tax, unit_cost, exchange_rate, moq } = item
  // item.margin = ((unit_price_no_tax - unit_cost * exchange_rate * moq) / (unit_cost * exchange_rate * moq)) * 100
}
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="8">
            <el-form-item label="貨幣 Cost Currency">
              <el-select v-model="item.cost_currency" class="full-width-input" clearable>
          ...
          <el-col :span="8">
            <el-form-item label="匯率 Unit Cost">
            ...
            <el-form-item label="成本 Exchange Rate">
              <el-input-number v-model="item.exchange_rate" controls-position="right" step-strictly disabled />
            </el-form-item>
