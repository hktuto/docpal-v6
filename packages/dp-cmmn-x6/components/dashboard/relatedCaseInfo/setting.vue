<template>
  ...
  >
    <el-form ref="formRef" :model="state.setting" label-position="top">
      <el-form-item :label="$t('caseManagement.name')" prop="caseId" :rules="{ required: true, message: getTip('caseManagement.name') }">
        <el-select v-model="state.setting.caseId" filterable :placeholder="$t('caseManagement.name')" @change="handleCaseChange">
          <el-option v-for="item in state.caseList" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item :label="$t('common_title')" prop="title" :rules="{ required: true, message: getTip('common_title') }">
        <el-input v-model="state.setting.title" :placeholder="$t('common_title')" />
      </el-form-item>

      <el-form-item
        :label="$t('caseManagement.id')"
...
const filterList = computed(() => {
  return state.relatedCaseFields
    .filter((item: any) => {
...
function getTip(name: string) {
  return t('render.hint.fieldRequired', { name: t(name) })
}
...
async function initOptions() {
  ...
  state.caseFields = await getCaseFields(versionId)
}
...
async function handleCaseChange(value: string, isInit: boolean = false) {
  const curItem = state.caseList.find((item: any) => item.value === value)
  if (!!curItem) {
    state.setting.title = curItem.label
    state.setting.caseLabel = curItem.label
    state.setting.caseVersionId = curItem.versionId
  }
  state.relatedCaseFields = await getCaseFields(state.setting.caseVersionId)
}
function handleOpen(setting: any) {
  ...
  setTimeout(async () => {
    ...
    state.loading = false
    if (!!state.setting.caseId) handleCaseChange(state.setting.caseId, true)
  })
}
