...
async function getMasterTableData() {
  try {
    ...
    const { data: masterTableRecord } = await newClientApi.postDmsMasterTableRecordPageNonpermission({
    ...
    return state.masterTableRecord
  } catch (error) {
...
const { settingRef, cardRef, refresh, loading } = useDashboardCard({
  ...
  handleInitCardAction: async (setting: any) => {
    const data = await getMasterTableData()
