...
import { globalApi, newClientApi } from 'api'
...
async function handleSubmit() {
  ...
  try {
    ...
    await globalApi.postCaseInstanceTasksComplete({
...
async function handleOpen(taskId, actionItem, actionList) {
  ...
  const data = await globalApi.getCaseInstanceTasksTaskidForm(taskId).then(r => r.data)
