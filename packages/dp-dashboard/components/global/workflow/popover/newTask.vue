...
async function workflowClickHandler(item: any) {
  ...
  const data = await $api.get(`https://132.148.160.191:8001/api/v1/workflow/definitions/instance/${item.id}`).then((r: any) => r.data)
  if (!data) return
  if (data.status === 'D') {
    state.loading = false
...
async function checkAndSubmit() {
  ...
  if (!!formData) {
    ...
    try {
      const data = await $api.post('https://132.148.160.191:8001/api/v1/processes', formParams).then((r: any) => r.data)
      state.formDialogVisible = false

      setTimeout(async () => {
        // Check workflow running status
        const newVar = await $api.get(`https://132.148.160.191:8001/api/v1/processes/instance/${data.id}`).then((r: any) => r.data)
