...
export const useRelation = () => {
  ...
  async function getMenuFromDb() {
    ...
    if (workspaceRouteParams.value.detailId) {
      list = filterTreeById(list, workspaceRouteParams.value.detailId)
    }
    menus.value = list
  }
...
function filterTreeById(menus: any[], menuId: string): any[] {
  const result = menus.filter((menu) => {
    if (menu.children) {
      menu.children = filterTreeById(menu.children, menuId)
    }
