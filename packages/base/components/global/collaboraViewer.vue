...
const css = ref('')
const ui = ref(
  `UITheme=${userPreference.value.color};UIMode=notebookbar;TextRuler=false;PresentationStatusbar=false;SpreadsheetSidebar=false;SavedUIState=false;TextSidebar=false;TextToolbar=false`
)
const token = ref('')
...
async function displayIframe() {
  ...
  token.value = await newClientApi
    .getGetofficetokenId(props.docId, {
      fileType: props.fileType || 'NUXEO'
    })
    .then((r) => r.data)
...
function toggleMode() {
  ...
  if (mode.value === 'view') {
    ...
    if (isModified.value) {
      ...
      // save
      xlsxIframe.value.contentWindow.postMessage(
        JSON.stringify({
...
<template>
  <el-button :type="mode === 'view' ? 'info' : 'primary'" :class="{ editToggleButton: true, iframeReady }" :disabled="!iframeReady" @click="toggleMode">
    {{ $t('collabora.' + mode) }}
    <SvgIcon v-if="isEditable" class="el-icon--right" :src="mode === 'view' ? '/icons/file/edit.svg' : '/icons/close.svg'"></SvgIcon>
  </el-button>
  <div class="xlsContainer">
    <form
      ...
      method="post"
      :target="'collabora-online-viewer' + timestamp"
    ...
    <iframe
      ...
      class="xlsxIframe"
      frameborder="0"
      :name="'collabora-online-viewer' + timestamp"
      allowfullscreen
    ></iframe>
  </div>
</template>
