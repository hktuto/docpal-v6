...
const { databaseMenuRouteParams, findItemById, menuState, saveMenuItemToDb } = useSingleDatabaseContext()
...
async function handleSaveGeneral() {
  try {
    loading.value = true
    await saveMenuItemToDb({ ...formData.value, id: databaseMenuRouteParams.value.detailId })
    const treeItem = findItemById(menuState.value.items, databaseMenuRouteParams.value.detailId || '')
    if (treeItem) {
