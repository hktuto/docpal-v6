...
import {
  ...
  type RelationJobStatus
} from '../../../../composables/import/useImportRelationEstablish'
import { newClientApi } from 'api'

const props = defineProps<{
  guess: RelationGuess | null
}>()

const emit = defineEmits<{
  (e: 'established', guess: RelationGuess): void
  (e: 'cancelled'): void
}>()
...
const pollStatus = ref<RelationJobStatus | null>(null)
...
async function loadTargetFields() {
  ...
  try {
    ...
    // Exclude system/internal fields that shouldn't be display fields
    targetFields.value = fields.filter(
      (f) => f.field_name && !['id', 'createdAt', 'updatedAt', 'createdBy', 'updatedBy'].includes(f.field_name)
    )
    // Default select the target match field + a couple of nice display fields
    const defaultIds = new Set<string>()
    if (guess.targetFieldId) defaultIds.add(guess.targetFieldId)
    // Add up to 2 additional fields that look like names
    ...
    // Default label to target table name
...
<template>
  <el-dialog
    ...
    @close="close"
  >
    <div v-if="guess" class="confirm-dialog-body">
      <!-- Relation summary -->
      <div class="relation-summary">
        <div class="summary-row">
          <span class="summary-label">From</span>
          <span class="summary-value">{{ guess.sourceTableName }} — {{ guess.sourceFieldAlias }}</span>
        </div>
        <div class="summary-row">
          <span class="summary-label">To</span>
          <span class="summary-value">{{ guess.targetTableName }} — {{ guess.targetFieldAlias }}</span>
        </div>
        <div class="summary-row">
