...
import type { WorkspaceRouteParams, TreeItem } from '../../../composables/database/useSingleDatabase'
...
const { databaseMenuRouteParams, database, menuState, navigateToItem, findItemById, goBackFromRecord } = useSingleDatabaseContext()
...
function handleBreadcrumbClick(item: BreadcrumbItem) {
  ...
  if (item.params.detailType === 'root') {
    ...
    if (databaseMenuRouteParams.value.detailType === 'record') {
      goBackFromRecord()
            ...
            <template #dropdown>
              <el-dropdown-menu v-if="item.children && item.children.length > 0" class="breadcrumb-dropdown-menu">
                ...
                <el-dropdown-item
                  ...
                  :key="child.id"
                  :command="child"
                  :class="{ 'is-current': child.id === databaseMenuRouteParams.detailId }"
                >
