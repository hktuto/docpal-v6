...
const displayUserList = computed(() => {
  ...
  return userGroups.value.filter(item => !hideList.find(oldItem => oldItem.attr_name === item.value))
})
...
onMounted(async () => {
  userGroups.value = await getGroupsSelectOption()
})
  ...
  <ElForm ref="formEl" :model="form" :rules="validationRules" label-position="top">
    <ElFormItem prop="attr_name" label="User Group" required>
      <ElSelect v-model="form.attr_name" filterable>
        <ElOption v-for="item in displayUserList" :key="item.value" :label="item.label" :value="item.value" />
      </ElSelect>
    </ElFormItem>
