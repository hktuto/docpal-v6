...
const { tableConfig, tableEvent, tableRef, query, reload, cleanSelectedRows } = useVxeTable({
  ...
  api: (pageParams: any) => clientApi.api.postDmsCompanyprofilesCompanyidChopsPage(props.id, {
...
async function handleActive(row: any, status: string) {
  try {
    const result = await clientApi.api.putDmsCompanyprofilesCompanyidChopsCompanychopidStatus(props.id, row.id, { status: status }).then((res) => res.data)
    if (!!result) {
      row.status = status
    }
  } catch (error) {
...
async function handleDelete(row: any) {
  try {
    ...
    await clientApi.api.deleteDmsCompanyprofilesCompanyidChopsCompanychopid(props.id, row.id).then((res) => res.data)
    reload()
  } catch (error) {
    console.log(error)
  }
