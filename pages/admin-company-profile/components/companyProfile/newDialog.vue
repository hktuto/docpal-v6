  ...
  <el-dialog v-model="state.visible" :title="$t('companyProfile.create')" class="scroll-dialog" append-to-body
    ...
    <template #footer>
      <div class="footer-grid">
        <el-button id="CompanyProfile__NewProfile__Submit" type="primary" :loading="state.loading"
                   @click="handleSubmit">
          {{ $t('common_submit') }}
        </el-button>
      </div>
...
async function handleSubmit() {
  try {
    ...
    state.loading = true
    await clientApi.admin.postAdmindmsCompanyprofiles(data).then(r => r.data)
