    ...
    switch (workflowType) {
      ...
      default:
        const workflowTaskInstance = await $api
          .get(`https://132.148.160.191:8001/api/v1/workflow/definitions/instance/${detail.definition_id}`)
          .then((r) => r.data)
        workflowJson.value = workflowTaskInstance.content

        const data = await $api.get(`https://132.148.160.191:8001/api/v1/processes/instance/${detail.process_instance_id}`).then((r) => r.data)
...
async function initForm(node: any) {
  ...
  // Get Form Data
  let formData = {}
  const taskDetailData = await $api.get(`https://132.148.160.191:8001/api/v1/tasks/instance/${detail.id}`).then((r) => r.data)
...
async function handleFormDataGet() {
  // Get Form Data
  await $api.get(`https://132.148.160.191:8001/api/v1/processes/variable/${id}/variables`).then((r) => r.data)
...
async function handleSubmit() {
  ...
  try {
    if (detail.assignee !== userId) {
      await $api
        .post(`https://132.148.160.191:8001/api/v1/tasks/instance/${taskDetail.value.id}/claim`, {
          user_id: userId,
...
async function handleSubmitUserTask() {
  ...
  const data = $api
    .post(`https://132.148.160.191:8001/api/v1/processes/instance/${detail.process_instance_id}/tasks/${taskDetail.value.id}/complete`, {
      user_id: userId,
      variables: fromData
    })
    .then((r) => r.data)
...
async function handleSubmitServiceTask() {
  ...
  const data = $api
    .post(`https://132.148.160.191:8001/api/v1/processes/instance/${detail.process_instance_id}/tasks/${taskDetail.value.id}/execute`, {
      variables: fromData
    })
    .then((r) => r.data)
