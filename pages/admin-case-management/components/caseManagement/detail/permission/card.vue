<template>
  <div v-if="!!data" class="permissionCard">
    ...
    <el-icon class="absoluteTop" @click="handleEdit">
      <Setting />
    </el-icon>
    <CaseManagementDetailPermissionDialog ref="dialogRef" :caseInformation="caseInformation" :exitList="enableExitList"
                                          @refresh="handleRefresh" @delete="(data) => emits('delete', data)" />
  </div>
</template>
...
const fieldList = computed(() => {
  try {
    return props.data.permission.field.reduce((prev, item) => {
