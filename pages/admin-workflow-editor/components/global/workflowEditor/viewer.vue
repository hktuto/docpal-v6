...
async function getData() {
    ...
    const xmlBlob = await clientApi.workflowVersionController.getBpmnxml({draftId:id, versionNumber:latestVersion}, {
        format: 'blob'
    }) as unknown as Blob
    const bpmn = await xmlBlob.text()
    const json = await clientApi.workflowVersionController.getJson({draftId:id, versionNumber:latestVersion}, {})
    if(json && json.data){
