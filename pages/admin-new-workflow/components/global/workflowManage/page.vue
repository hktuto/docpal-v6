...
async function getData(params: any) {
  const data = await $api.post('http://132.148.160.191:8001/api/v1/workflow/definitions/page', params).then((r) => r.data)
  return {
    data: {
...
async function handleActiveAndInactive(row: any, status: boolean) {
  try {
    if (status) {
      ...
      await $api.put(`http://132.148.160.191:8001/api/v1/workflow/definitions/instance/${row.id}/activate`, { user_id: userId.value }).then((r) => r.data)
    } else {
      await $api.put(`http://132.148.160.191:8001/api/v1/workflow/definitions/instance/${row.id}/deactivate`).then((r) => r.data)
    }
    reload()
  } catch (error) {
    console.error(error)
...
async function handleRemove(row: any) {
  ...
  try {
    await $api.delete(`http://132.148.160.191:8001/api/v1/workflow/definitions/instance/${row.id}`).then((r) => r.dada)
    reload()
  } catch (e) {
    console.log('')
  }
