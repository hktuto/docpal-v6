...
async function handleSubmit() {
  try {
    // Edit Workflow Info
    if (isEdit) {
      const workflowJson = form.value.draft_content
      workflowJson.name = formData.value.name
      workflowJson.description = formData.value.description

      $api.put(`/api/v1/workflow/definitions/instance/${formData.value.id}`, workflowJson).then((res) => res.data)
      emits('refresh')
      return
    }
