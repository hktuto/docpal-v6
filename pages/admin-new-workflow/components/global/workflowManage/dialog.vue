...
async function checkWorkflowName(rule: any, value: string, callback: any) {
  const data: any = await $api.get(`https://132.148.160.191:8001/api/v1/workflow/definitions?name=${formData.value.name}`).then((r) => r.data)
  if (data.length > 0) {
...
async function handleSubmit() {
  try {
    ...
    if (isEdit.valuel) {
      ...
      await $api.put(`https://132.148.160.191:8001/api/v1/workflow/definitions/instance/${formData.value.id}`, workflowJson).then((res) => res.data)
      emits('refresh')
      return
    }
    ...
    const data = await $api.post('https://132.148.160.191:8001/api/v1/workflow/definitions', defWorkflowJson).then((res) => res.data)
    if (!data) return
