...
export interface FormViewConfig {
  ...
  /** Label width in pixels (when labelPosition is 'left') */
  labelWidth?: number
...
export function generateDefaultFormConfig(fields: FieldInfo[]): FormViewConfig {
  // System field types that should be excluded from forms
  const systemFieldTypes = new Set([
    21, // CreatedTime
    22, // LastModifiedTime
    23, // CreatedBy
    24 // LastModifiedBy
  ...
  const formFields = fields.filter(
    (f) =>
      !f.isSystem &&
      !systemFieldTypes.has(f.type) &&
      f.type !== 6 && // Attachment/Document (handled separately)
      f.type !== 14 && // Relation
      f.type !== 16 && // Formula (read-only)
      f.type !== 15 // Aggregation/VirtualColumn (read-only)
  )
