...
onMounted(() => {
  ...
  permissionState.tableData = [
    { userId: 'user1@example.com', type: 'user', read: true, write: true, delete: false, loading: false },
    { userId: 'Editor Group', type: 'group', read: true, write: true, delete: false, loading: false }
  ]
})
</script>
...
<template>
  <el-card class="setting-section" v-loading="permissionState.loading">
    ...
    <div class="description">
      Manage who can access this table
    </div>
        ...
        <el-table-column label="Delete" align="center" width="100">
          <template #default="{ row }">
            <el-switch
              v-model="row.delete"
              :loading="row.loading"
              @change="(value: boolean) => handlePermissionChange(value, 'delete', row)"
            />
          </template>
        </el-table-column>
