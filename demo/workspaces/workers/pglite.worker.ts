 ...
 */
async function getMigrationList(): Promise<string[]> {
  try {
    const response = await fetch("/migrations/meta/_journal.json")
    if (response.ok) {
      const journal = await response.json()
      return journal.entries.map((entry: any) => `${entry.tag}.sql`)
    }
  } catch (error) {
 ...
 */
async function loadMigrationFile(filename: string): Promise<string> {
...
async function recordMigration(hash: string): Promise<void> {
  ...
  try {
    ...
    await db.exec(
      `INSERT INTO __drizzle_migrations (hash) VALUES ('${escapedHash}') ON CONFLICT (hash) DO NOTHING`
    )
  } catch (error) {
 ...
 */
async function applyMigrationsInternal(): Promise<void> {
  if (!db) {
