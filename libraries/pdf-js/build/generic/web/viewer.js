...
const PDFViewerApplication = exports.PDFViewerApplication = {
  ...
  async download() {
    const url = this._downloadUrl,
      filename = this._docFilename.replace('.pdf', '(annotation).pdf');
    try {
...
const RendererType = exports.RendererType = {
  CANVAS: "canvas",
  SVG: "svg"
};
...
class Toolbar {
  ...
  #bindListeners(options) {
    ...
    for (const {
      ...
      const eventHandlerIndex = window._eventHandler.findIndex(item => item.element === element && item.evHandler.toString() === evHandler.toString());
      if (eventHandlerIndex > -1) {
        element.removeEventListener("click", window._eventHandler[eventHandlerIndex].evHandler);
...
function webViewerLoad(url) {
  ...
  _app.PDFViewerApplication.run(config, url);
}
document.blockUnblockOnload?.(true);
document.onreadystatechange = function () {
  if ((document.readyState === "interactive" || document.readyState === "complete") && !window.pdfReady) {
...
function messageFromParent(ev) {
  ...
  const els = [document.querySelector("#secondaryPrint"), document.querySelector("#print"), document.querySelector("#secondaryDownload"), document.querySelector("#download")];
  ...
  if (options.readOnly === true) {
    const hiddenEL = [document.querySelector("#editorModeButtons"), document.querySelector("#saveAnnotation"), document.querySelector("#print"), document.querySelector("#editorFreeText"), document.querySelector("#editorInk")];
